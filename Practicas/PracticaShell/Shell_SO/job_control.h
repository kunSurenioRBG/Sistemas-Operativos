...
#define restore_terminal_signals()   terminal_signals(SIG_DFL) //cuando el proceso hijo hace "exit()", el proceso padre finaliza su waitpid() y recupera la terminal
#define ignore_terminal_signals() terminal_signals(SIG_IGN) //cuando un proceso en primer plano tienes el teminal, ignora el resto de seniales relacionadas con el terminal

#define set_terminal(pid)        tcsetpgrp (STDIN_FILENO,pid) //shell asigna el uso de terminal de forma apropiada, para evitar indeterminaciones entre tareas que intenten de leer a la vez del terminal
#define new_process_group(pid)   setpgid (pid, pid) //permite poner un nuevo proceso en otro grupo de procesos
